...
class DDList(Frame):
    def __init__(self, master, item_width, item_height, item_relief=None, item_background='red', item_borderwidth=None, offset_x=0, offset_y=0, gap=0, **kwargs):
...
if __name__ == "__main__":
    ...
    root.geometry("%dx%d%+d%+d"%(640, 1000, 0, 0))
    ...
    for i in range(len(cols)):
        item = sortable_list.create_item(value=cols[i])
        label = Label(item, text=cols[i])
        label.pack(anchor=W, padx= (4,0), pady= (4,0))

        initialSequence[cols[i]] = i
